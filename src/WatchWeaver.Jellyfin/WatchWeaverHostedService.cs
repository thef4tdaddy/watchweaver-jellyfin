...
{
    private readonly IServerApplicationHost _host;private readonly IApplicationPaths _paths;private readonly IUserDataManager _userData;private readonly IUserManager _users;private readonly ILogger<WatchWeaverHostedService> _log;private OutboundQueue? _queue;private Dispatcher? _dispatcher;private readonly EventCorrelation _correlation;public static WatchWeaverHostedService? Instance{get;private set;}
    public WatchWeaverHostedService(IServerApplicationHost host,IApplicationPaths paths,IUserDataManager userData,IUserManager users,EventCorrelation correlation,ILogger<WatchWeaverHostedService> log){_host=host;_paths=paths;_userData=userData;_users=users;_correlation=correlation;_log=log;Instance=this;}
    public override async Task StartAsync(CancellationToken ct){var folder=Path.Combine(_paths.PluginConfigurationsPath,"watchweaver");_queue=new(Path.Combine(folder,"outbound-queue.json"),Plugin.Instance?.Configuration.QueueCapacity??10000);await _queue.LoadAsync(ct);_dispatcher=new(new HttpClient{Timeout=TimeSpan.FromSeconds(20)},_queue,Configuration);_userData.UserDataSaved+=OnUserDataSaved;await base.StartAsync(ct);}
    ...
    private void OnUserDataSaved(object? sender,UserDataSaveEventArgs e){try{var user=_users.GetUserById(e.UserId);if(e.Item is not null&&user is not null&&e.UserData.Played&&e.SaveReason.ToString().Contains("UpdateUserRating",StringComparison.OrdinalIgnoreCase))Capture(e.Item,user,null,_correlation,"marked_played");}catch(Exception ex){_log.LogWarning(ex,"WatchWeaver manual watched-state capture failed");}}
    public async void Capture(BaseItem item,global::Jellyfin.Database.Implementations.Entities.User user,SessionInfo? session,EventCorrelation correlation,string type){try{var cfg=Plugin.Instance?.Configuration;if(cfg is null||_queue is null||!cfg.AllowedUserIds.Contains(user.Id.ToString(),StringComparer.OrdinalIgnoreCase))return;var data=_userData.GetUserData(user,item);var now=DateTimeOffset.UtcNow;var eventId=correlation.GetEventId(_host.SystemId,user.Id.ToString(),item.Id.ToString(),data.PlayCount,now,type);var isEpisode=item is MediaBrowser.Controller.Entities.TV.Episode;var episode=item as MediaBrowser.Controller.Entities.TV.Episode;var providers=(item.ProviderIds??new Dictionary<string,string>()).ToDictionary(x=>x.Key.ToLowerInvariant(),x=>x.Value);var envelope=new EventEnvelope(1,eventId,type,now,new(_host.SystemId,_host.ApplicationVersionString),new(typeof(Plugin).Assembly.GetName().Version?.ToString()??"0.1.0",TargetAbi()),new(user.Id.ToString(),user.Username),new(item.Id.ToString(),isEpisode?"episode":"movie",item.Name??"Unknown",item.ProductionYear,episode?.SeriesName,episode?.ParentIndexNumber,episode?.IndexNumber,providers),new(true,data.PlaybackPositionTicks,item.RunTimeTicks,item.RunTimeTicks>0?100d*data.PlaybackPositionTicks/item.RunTimeTicks:data.Played?100:0,data.PlayCount,session?.Client,session?.DeviceName));if(!await _queue.EnqueueAsync(envelope))_log.LogError("WatchWeaver outbound queue is full; event was not accepted into the queue");}catch(Exception ex){_log.LogWarning(ex,"WatchWeaver event capture failed without exposing event data");}}
