...
public sealed class Dispatcher(HttpClient client,OutboundQueue queue,Func<(Uri? Url,string Token)> configuration)
{
    ...
    internal static TimeSpan Backoff(int attempts,double? jitter=null){var baseSeconds=Math.Min(3600,Math.Pow(2,Math.Min(attempts,10))*5);var factor=jitter??(0.8+Random.Shared.NextDouble()*0.4);return TimeSpan.FromSeconds(Math.Min(3600,baseSeconds*factor));}
